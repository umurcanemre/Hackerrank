...
public class W1D5 {
	public static int maxProfit(int[] prices) {
		...
		for (int i = 1; i < prices.length && buyPos < prices.length; i++) {
			if (buyPos == 0 && sellPos == 0 && prices[i] <= prices[buyPos]) {
				buyPos = i;
				sellPos = i;
			}
			if (prices[i] > prices[buyPos] && prices[i] > prices[sellPos]) {
				sellPos = i;
			}
			if (prices[i] < prices[sellPos]) {
				result += prices[sellPos] - prices[buyPos];
				buyPos = i;
				sellPos = i;
			}
